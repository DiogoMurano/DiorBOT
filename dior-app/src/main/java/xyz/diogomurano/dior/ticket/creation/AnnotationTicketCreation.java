...
public class AnnotationTicketCreation implements TicketCreation {
        ...
        process.add(new Step() {
            ...
            public MessageEmbed getMessage() {
                return DiscordAPI.createEmbed(embedBuilder -> {
                    embedBuilder.setTitle("Anotação - Etapa #1");
                    embedBuilder.setColor(color);
                    embedBuilder.setDescription("**Qual o nickname do jogador que deseja anotar?**");
        ...
        process.add(new Step() {
            ...
            public MessageEmbed getMessage() {
                return DiscordAPI.createEmbed(embedBuilder -> {
                    ...
                    embedBuilder.setTitle("Anotação - Etapa #2");
                    embedBuilder.setThumbnail("https://www.habbo.com/habbo-imaging/avatarimage?figure=" + user.getFigureString());
                    embedBuilder.setColor(color);
                    embedBuilder.setDescription("**Este é o usuário que deseja anotar?**");
        ...
        process.add(new Step() {
            ...
            public MessageEmbed getMessage() {
                return DiscordAPI.createEmbed(embedBuilder -> {
                    ...
                    embedBuilder.setTitle("Anotação - Etapa #3");
                    embedBuilder.setThumbnail("https://www.habbo.com/habbo-imaging/avatarimage?figure=" + user.getFigureString());
                    embedBuilder.setColor(color);
                    embedBuilder.setDescription("**Qual a razão da anotação?**");
        ...
        process.add(new Step() {
            ...
            public MessageEmbed getMessage() {
                return DiscordAPI.createEmbed(embedBuilder -> {
                    ...
                    embedBuilder.setTitle("Anotação - Etapa #4");
                    embedBuilder.setThumbnail("https://www.habbo.com/habbo-imaging/avatarimage?figure=" + user.getFigureString());
                    embedBuilder.setColor(color);
                    embedBuilder.setDescription("**Confirma o envio desta anotação?**");
                    embedBuilder.addField("Nickname:", user.getName(), false);
            ...
            public void validateReaction(MessageReaction reaction) {
                ...
                if (emoji.equalsIgnoreCase("U+2705")) {
                    ...
                    metadata.store("lastDate", lastDate);
                    channel.sendMessage(DiscordAPI.createEmbed(embedBuilder -> {
                        embedBuilder.setTitle("Anotação de " + user.getName());
                        embedBuilder.setThumbnail("https://www.habbo.com/habbo-imaging/avatarimage?figure=" + user.getFigureString());
                        embedBuilder.setColor(color);
                        embedBuilder.addField("Autor:", reaction.getGuild().getMemberById(ticket.getAuthorId()).getNickname(), false);
                        embedBuilder.addField("Nickname:", user.getName(), false);
                        embedBuilder.addField("Cargo atual:", collaborator.getRole().getName(), false);
                        embedBuilder.addField("Descrição:", metadata.get("description"), false);
                    })).queue(message -> {
                    ...
                    channel.sendMessage(DiscordAPI.createEmbed(embedBuilder -> {
                        embedBuilder.setTitle("Anotação solicitada!");
                        embedBuilder.setDescription("A anotação foi publicada com sucesso. Este ticket foi definido como concluído!");
                        embedBuilder.addField("Status:", "Aguarde algum membro da DIRETORIA " +
                                "confirmar o envio desta anotação.", false);
                    ...
                    channel.sendMessage(guild.getRoleById("708843250789908541").getAsMention()).delay(1, TimeUnit.SECONDS)
                            .queue(message -> message.delete().queue());

                } else {
                    channel.sendMessage(DiscordAPI.createEmbed(embedBuilder -> {
                        embedBuilder.setTitle("Anotação cancelada!");
                        embedBuilder.setDescription("A anotação foi cancelada. Caso necessário, abra um novo ticket!");
                        embedBuilder.setColor(guild.getSelfMember().getColor());
                    })).queue();
                    ticketHolder.archiveTicket(ticket, reaction.getGuild().getSelfMember(), "Anotação cancelada por solicitação do usuário!");
                }
            }
